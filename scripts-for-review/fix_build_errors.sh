#!/usr/bin/env bash
# fix_build_errors.sh - Fix TypeScript compilation errors
set -euo pipefail
...
# Fix 1: Update Sections.tsx to accept id prop
cat > src/components/Sections.tsx << 'EOF'
        ...
        <div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-neutral">8735 Dunwoody Place, Suite N<br />Atlanta, GA 30350</p>
            </div>
          </div>
        </div>
        <div>
    ...
    <div className="container">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
        ...
        <div>
          ...
          <ul className="space-y-2 text-gray-300">
            <li><a href="mailto:hello@badgrtech.com" className="hover:text-white transition-colors">Email</a></li>
            <li><a href="tel:+14044235493" className="hover:text-white transition-colors">Phone</a></li>
          </ul>
        </div>
      </div>
...
echo "✓ Sections.tsx fixed"

# Fix 2: Remove quote calculator page references (they don't exist yet)
cat > src/App.tsx << 'EOF'
import React from 'react';
...
# Fix 3: Remove broken quote page files if they exist
rm -f src/pages/BrandingQuotePage.tsx 2>/dev/null || true
rm -f src/pages/VisualQuotePage.tsx 2>/dev/null || true

echo "✓ Removed broken quote page references"

# Test build again
echo ""
echo "Testing build..."
if npm run build 2>&1 | tee final-build.log; then
    echo ""
    echo "✅ BUILD SUCCESSFUL"
else
    echo ""
    echo "❌ Build still failing - check final-build.log"
    exit 1
fi
...
echo "╚════════════════════════════════════════════════════════════╝"
echo ""
echo "Your site is now ready with Cloudinary images!"
echo ""
echo "Start dev server: npm run dev"
echo "Run tests: ./pre_launch_compliance_tests.sh"